...
par.base_path = '/home/yudongyao/Documents/SSRF_13HB_20240506/20240505/sample2_3_db_0.650um_600ms_10x_285mm_contrast/SMDF_results/'; %dark filed & phase
% par.base_path = '/home/yudongyao/Documents/SSRF_13HB_20240506/13hb_20240506_preprocess/sample1_2_6k_0.325um_1000ms/'; %background not clean
% par.base_path = '/home/yudongyao/Documents/SSRF_13HB_20240506/13hb_20240506_preprocess/sample2_1_db_0.325um_700ms/';% RC not in FOV
% par.base_path = '/home/yudongyao/Documents/SSRF_13HB_20240506/13hb_20240506_preprocess/sample2_3_db_0.325um_1000ms/'; % RC not in FOV

% filename = 'tomo_clean_dark.mat'; %absorption
% filename = 'thickness_tomo.mat';%phase 
filename = 'DF_tomo.mat';%dark-field 

% filename = 'tomo_cut.mat';
par.output_folder = fullfile(par.base_path,'results/DF/');
...
par.GPU_list = [7];     % number of the used GPU % If you want to check usage of GPU 
                        % > nvidia-smi
...
par.lamino_angle = 65.2113;        % laminography angle, should be 90 for common tomo
par.tilt_angle = -0.1889;         % rotation of the camera around the beam direction, should be 0 for common tomo 
par.skewness_angle = 1.0598;        % shear of the "camera axis" , should be 0 for common tomography
...
tomo.show_projections(projs_data.tomo, theta, par, 'title', 'Full original projections before alignmnent')


%%
% stack_object = projs_data.tomo(1546-500+1:1546+500,806-800+1:806+800,:);%absorption
stack_object = projs_data.tomo(1442-500+1:1442+500,797-750+1:797+750,:);% phase

%remove background
for jj=1:size(stack_object,3)
    stack_object(:,:,jj) = stack_object(:,:,jj) - stack_object(1,1,jj);
end
utils.verbose(-1,'Done')

% stack_object = exp(1j*stack_object);
stack_object = exp(1j*stack_object/30);%DF


tomo.show_projections(angle(stack_object), theta, par, ...
...
% remove projections with total_shift>10 

% p_total_shift = load('/home/yudongyao/Documents/SSRF_13HB_20240506/20240505/sample2_3_db_0.650um_600ms_10x_285mm_contrast/SMDF_results/results/TIFF_delta_projs720_lanmi_angle65_FBP_highpass0.03_CoR_offset32.5/total_shift.mat');
% index1 = find(abs(p_total_shift.total_shift(:,1))>40);
% index2 = find(abs(p_total_shift.total_shift(:,2))>40);
% which_remove = vertcat(index1,index2);
...
%%% Edit this section %%%
%%%%%%%%%%%%%%%%%%%%%%%%%
par.filter_pos = 10;        %  highpass filter on the evolution of the recovered positions applied in the following form:  X - smooth(X,par.filter_pos), it prevents accumulation of drifts in the reconstructed shifts 
...
%%
% very important
% CoR_offset_v = -19; %rotation center - center of fov
% CoR_offset_offset = 68; % depends on figure 9 find RC

% CoR_offset_v = -9;
% CoR_offset_offset = 19;

% CoR_offset_v = -9;
% CoR_offset_offset = 13;
...
par.high_pass_filter = 0.03;        % remove effect of residuums/bad ptycho convergence , high value may get stuck in local minima 
...
par.mask_threshold = 0.001;         % []; % empty == O tsu thresholding 
...
reconstruct_ROI2 = {reconstruct_ROI{1}(1)+reconstruct_ROI_offset:fix(reconstruct_ROI{1}(end)/2)*2-reconstruct_ROI_offset,reconstruct_ROI{2}(1)+reconstruct_ROI_offset:fix(reconstruct_ROI{2}(end)/2)*2-reconstruct_ROI_offset};
clear sinogram 
...
max_sino = tomo.block_fun(@(x)max(x,[],3),sinogram,struct('use_GPU', false, 'reduce_fun', @max, 'ROI', {reconstruct_ROI2})); 

f = plotting.smart_figure(9);
clf()
imagesc(reconstruct_ROI2{2},reconstruct_ROI2{1},max_sino)
hold on 
axis image xy
colormap bone
grid on 
...
hold on; plot(mean(reconstruct_ROI2{2}),mean(reconstruct_ROI2{1}),'xy'); hold off 
drawnow


%% 
% CoR_offset = param.center_of_rotation(2) - ...
%        (length(reconstruct_ROI2{2})/2 -0.5 + ... 
%             reconstruct_ROI2{2}(1)-1);
% CoR_offset_v_recon=0;
% CoR_offset_v_recon = 502-length(reconstruct_ROI2{1})/2; % depends on figure 9 find RC
% CoR_offset = 869-length(reconstruct_ROI2{2})/2;

% CoR_offset_v_recon = 510-length(reconstruct_ROI2{1})/2; % depends on figure 9 find RC %not important?
% CoR_offset = 772-length(reconstruct_ROI2{2})/2-4;

CoR_offset_v_recon = 489-length(reconstruct_ROI2{1})/2; % depends on figure 9 find RC %not important?
CoR_offset = 763-length(reconstruct_ROI2{2})/2;
...
%% save tomogram
par.scans_string = 'projs720_lanmi_angle65';
par.rec_delta_info = 'FBP_highpass0.03_CoR_offset27';
par.force_overwrite = true;
% tomo.save_tomogram(rec, par, 'delta', circulo, theta, par.rec_delta_info)
tomo.save_tomogram(tomogram, par, 'delta', circulo, theta, par.rec_delta_info)
...
par.name_prefix = 'invert_tomo_delta';
%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%
% tomo.save_as_tiff(rec, par, par.rec_delta_info)

tomogram_invert = max(tomogram(:))-tomogram;
tomo.save_as_tiff(tomogram_invert, par, par.rec_delta_info)

%% save total_shift
save(fullfile(par.output_folder,par.tiff_subfolder_name,'total_shift.mat'),'total_shift')