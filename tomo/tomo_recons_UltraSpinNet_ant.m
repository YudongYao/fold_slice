...
par.base_path = 'C:\Users\yudongyao\Work\Data\UltraSpinNet\ant\';
filename = 'projection_D.hdf5';
par.output_folder = 'C:\Users\yudongyao\Work\Data\UltraSpinNet\ant\D\';
...
par.air_gap = [100,100];    % very roughly distance around sides where is assumed air, it is used only for initial guess       
...
%% 
stack_object(722:767,252:290,:)=stack_object(722-50:767-50,252:290,:);

% stack_object = exp(1j*log(stack_object/500+1));
% stack_object = exp(1j*log(stack_object/3+1));
...
par.high_pass_filter = 0.05;        % remove effect of residuums/bad ptycho convergence , high value may get stack in local minima , low value increases sensitivity to residua / phase jumps 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sinogram = tomo.unwrap2D_fft2_split(stack_object, par.air_gap ,1, struct('GPU_list', par.GPU_list, 'ROI', {object_ROI}, 'use_fp16', false));
...
tomo.align_tomo_global_parameters(sinogram,theta(:), Npix_align, par, 'binning', 4, ...
'search_range',  [-1, 1] , 'num_grid_points', 20, ...
'search_parameter',  'center_of_rotation', ... %% choose the searched parameter
...
for jj = 1:length(binning)
    ...
    [shift, par] = tomo.align_tomo_consistency_linear(sinogram,sino_weights, theta+0.1, Npix, shift, par, 'CoR_offset', CoR_offset); 
...
CoR = [Ny_sino,Nx_sino]/2+[0,0.5]*strcmpi(par.phase_unwrapping,'none')+[0,CoR_offset];   % there is 0.5px shift between fft_1d/fft_2d vs none
