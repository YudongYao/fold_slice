...
par.GPU_list = [5];     % number of the used GPU % If you want to check usage of GPU 
                        % > nvidia-smi
...
Nlayers = floor(length(object_ROI{1})/64)*64;   %0  JD
...
binning = [32,16,8,4,2,1];
%binning = [2];

par.GPU_list = [4,5,6,7];     % use more GPUs if run out of memory
...
%%
utils.verbose(-1,'Reconstructing tomogram...Done')
[~,ind_sort] = sort(theta); 
