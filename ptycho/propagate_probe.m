...
addpath('C:\Users\yudongyao\Work\code\Ptychoshelves_PSI\')
import utils.*
...
rangez = [-20e-3 20e-3];      % Range in meters, currently at zero
...
mask=ones(asize,asize) - (abs(probe(:,:,probe_mode))==0);
%probe_corr=rmphaseramp(probe,mask);
% probe_corr=remove_linearphase_v2(probe,mask,20);
%probe=probe_corr;
probe_hsv(:,:,3)=abs(probe(:,:,probe_mode))/max(max(abs(probe(:,:,probe_mode))));
probe_hsv(:,:,2)=ones(asize,asize);
probe_hsv(:,:,1)=angle(probe(:,:,probe_mode))/(2*pi)+0.5;
figure(1); 
clf
switch disp
    case 'hsv'
    ...
    case 'amp'
        imagesc(x*1e6,x*1e6,abs(probe(:,:,probe_mode)));
        colormap bone
    case 'phase'
        imagesc(x*1e6,x*1e6,angle(probe(:,:,probe_mode)));
        colormap bone
end
axis xy equal tight; 
...
back_propag_all = prop_free_nf(probe(:,:,probe_mode), lambda, propdists, pixsize);

% propag = zeros(size(probe,1),step_num);
% for i=1:size(probe,3)
%     back_propag_all = prop_free_nf(probe(:,:,i), lambda, propdists, pixsize);
%     propag = propag+squeeze(back_propag_all(asize/2,:,:)); 
% end

% back_propag_all = zeros(500,500,length(propdists));
% for i=1:length(propdists)
%    back_propag_all(:,:,i) =  prop_free_ff(probe, lambda, propdists(i), pixsize);
% end
% back_propag_all = prop_free_ff(probe, lambda, propdists, pixsize);
...
back = prop_free_nf(probe(:,:,probe_mode), lambda, prop_dis, pixsize);
% back = prop_free_ff(probe, lambda, prop_dis, pixsize);
